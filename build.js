...
class Build {
    extend = [".d.ts", ".map", "meta.json", ".md"]; // 排除文件或拓展名
    path = ["dist", "assets"]; // 模块所在目录
    resource = []; // 以@resource形式加载的模块
    cdn = "https://cdn.jsdelivr.net/gh/MotooriKashin/BaiduYun-Escape"; // 在线仓库
    /**
     * 构建入口
     */
    async output() {
        ...
        const modules = files.reduce((s, d, i) => {
            ...
            s += arr[i].endsWith(".json") ? `\r\n/**/modules["${t[t.length - 1]}"] = /*** .${arr[i].slice(1)} ***/\r\n${String(d)}\r\n/*!***********************!*/` :
                arr[i].endsWith(".js") ? `\r\n/**/modules["${t[t.length - 1]}"] = /*** .${arr[i].slice(1)} ***/\r\n\`${String(d).replace(/\\/g, '\\\\').replace(/`/g, '\\`').replace(/\$/g, '\\$')}\r\n//# sourceURL=API://@${arr[i].slice(2).replace("dist", "BaiduYun-Escape")}\`;\r\n/*!***********************!*/` :
                    `\r\n/**/modules["${t[t.length - 1]}"] = /*** .${arr[i].slice(1)} ***/\r\n\`${String(d).replace(/\\/g, '\\\\').replace(/`/g, '\\`').replace(/\$/g, '\\$')}\`;\r\n/*!***********************!*/`
            return s;
        }, "");
